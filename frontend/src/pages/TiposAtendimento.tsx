...
export default function TiposAtendimento() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Tem certeza que deseja remover este tipo de atendimento?')) return

    setDeleteError(null)
    try {
      await deleteMutation.mutateAsync(id)
    } catch {
      setDeleteError('Erro ao remover tipo de atendimento.')
    }
  }
              ...
              <CardContent className="space-y-3">
                ...
                {podeGerenciar && (
                  <div className="flex gap-2 mt-4 justify-end">
                    {/* Visualizar abre o formulário de edição em modo leitura */}
                    <Button
                      size="sm"
                      variant="outline"
                    ...
                    <Button
                      size="sm"
                      variant="destructive"
                      disabled={deleteMutation.isPending || !tipo.ativo}
